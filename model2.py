    ...
    with open(CKPT_LOCK_PATH, "a+") as clf, open(GRAD_LOCK_PATH, "a+b") as gf:
        ...
        try:
            ...
            if apply_update:
                ...
                # Reconcile master params against the concurrent single-track models via weighted blending
...
def get_full_track_data(samples_per_sec=44100, num_diffusion_steps=50):
    ...
    while True:
        ...
        return (
            np.stack(patches)[None, :, :], # Shape (1, T, L)
...
def run_single_track_worker(worker_idx, initial_params):
    ...
    print(f"[SingleTrack-{worker_idx}] Initializing full track training target...")
    track_data, scale, bpm, stem, steps, sigmas, yt_id = get_full_track_data()
    print(f"[SingleTrack-{worker_idx}] Locked onto track {yt_id} (Length: {track_data.shape[1]}s). Training until loss hits 0...")
    ...
    while True:
        ...
        if step % 25 == 0:
            print(f"[SingleTrack-{worker_idx}] Track {yt_id} | Step {step} | Loss: {float(loss_val):.6f}")
            
        if float(loss_val) <= 1e-6:
            print(f"[SingleTrack-{worker_idx}] Target hit! Loss reached 0 on track {yt_id}.")
            break
...
if __name__ == "__main__":
    ...
    if "--train" in sys.argv:
        print(f"[Trainer] Launching {CONCURRENT_MODELS} single-track concurrent model workers...")
        ...
        threads = [threading.Thread(target=spawn_worker, args=(i, params)) for i in range(CONCURRENT_MODELS)]
        for t in threads: t.start()

        print(f"[Trainer] Resuming master variable-length training loop at global step {global_step} (Version {version}).")
